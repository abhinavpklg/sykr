...
export default function DashboardPage() {
  const { loading: actionsLoading, states, counts } = useJobActionsContext();
    ...
    <div className="min-h-screen">
      ...
      <main className="mx-auto max-w-7xl px-4 py-6">
        ...
        <div className="flex flex-wrap items-center gap-4 mb-6">
          <h1 className="text-xl font-bold text-t-primary">My Applications</h1>
          <div className="flex items-center gap-3 ml-auto">
            <div className="flex items-center gap-1.5 text-sm">
              <span className="inline-block h-2 w-2 rounded-full bg-accent" />
              <span className="text-t-secondary">{counts.saved} saved</span>
            </div>
            <div className="flex items-center gap-1.5 text-sm">
              <span className="inline-block h-2 w-2 rounded-full bg-green-bright" />
              <span className="text-t-secondary">{counts.applied} applied</span>
            </div>
            <div className="flex items-center gap-1.5 text-sm">
              <span className="inline-block h-2 w-2 rounded-full bg-t-muted" />
              <span className="text-t-secondary">{counts.hidden} hidden</span>
            </div>
          </div>
        </div>
          ...
          {TABS.map((tab) => (
            <button
              ...
              className={`px-4 py-2.5 text-sm font-medium transition-colors relative ${
                activeTab === tab.key ? "text-t-primary" : "text-t-muted hover:text-t-secondary"
              }`}
            >
              {tab.label}
              <span className={`ml-1.5 text-xs ${activeTab === tab.key ? "text-accent" : "text-t-muted"}`}>
                {tabCount(tab.key)}
              </span>
              {activeTab === tab.key && (
        ...
        {loadingJobs ? (
          ...
          <div className="flex flex-col items-center justify-center rounded-lg border border-dashed border-border py-20 text-center">
            ...
            <h3 className="mt-4 text-lg font-medium text-t-primary">
              {activeTab === "all" ? "No tracked jobs yet" : `No ${activeTab} jobs`}
            </h3>
